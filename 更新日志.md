...
Docker地址：queyouquan.tencentcloudcr.com/dev/real-name-authentication:v11
...
​	apollo 添加Redis配置 ，添加Consul配置

​	mj_user数据库tb_realname表添加uid字段的索引，新增tb_realname_pending表

# 正式服 更新 实名认证服务(国内)

Docker地址：queyouquan.tencentcloudcr.com/pro/real-name-authentication:v1.0.4
Apollo地址：http://apollo.mayouquan.net:8070/config.html#/appid=RealNameAuth&env=DEV&cluster=default
...
​	迁移完成后将logapi里web.config里的实名认证地址改成现在实名认证服务的地址
